...
Some sub-compounds appear in a fossilized possessive construction, such as \emph{phak\-kusa} \rede{pork}, literally \rede{pig's meat} or \emph{wagusa} \rede{chicken meat}, literally \rede{chicken's meat}.
 ...
 In the rather complex \isi{kinship} system with frequent instances of obligatory possession (cf. §\ref{inh-poss}), the prefixes marking possession usually attach to the first noun, as in  \emph{a-cya-mamu} \rede{daughter (my child + girl)} and  \emph{a-yem-namma} \rede{father-in-law's elder brother's wife} (my father's elder brother's wife + female in-law). Exceptions are found in the terminology for in-laws on the cousin level, e.g., \emph{khoknima-a-ŋoʈeŋma} \rede{father-in-law's sister’s daughter who is younger than EGO (father's sister's younger daughter + my-female-in-law)}.
 
 
\subsubsection{Toponyms}
 
Among the \isi{toponyms}, oronyms usually end in \emph{luŋ} (PTB *r-luŋ for \rede{stone}, \citealt[50]{Matisoff2003Handbook}). Examples are \emph{Taŋwaluŋ} (Mt. Makalu), \emph{Comluŋ} (Mt. Everest), \emph{Phakʈaŋluŋ}  (shoulder-rock, Mt. Kumbhakarna) or \emph{Namthaluŋma} (locally important rocks, connected to a mythical story).

Another \isi{syllable} appearing in \isi{toponyms} is \emph{liŋ}. It  is most probably related to PTB *b-liŋ for  \rede{forest/field} \citep[280]{Matisoff2003Handbook} and occurs in names of Yakkha villages, e.g., \emph{phakliŋ} (pig-field) or \emph{mamliŋ}  (big field), as it does in \isi{toponyms} of other \isi{Tibeto-Burman} languages, too.

\isi{Tibeto-Burman} languages often have locational nominalizers referring to a place connected to some noun, e.g., in Classical Tibetan \citep[300]{Beyer1992_Tibetan}. In Kiranti languages, one finds e.g., \emph{-khom \ti -khop} in \ili{Thulung} (cognate to Yakkha \emph{kham} \rede{ground}), and \emph{-dɛn} in \ili{Limbu} (cognate to Yakkha \emph{ten} \rede{village}, \citealt[89]{Ebert1994The-structure}). Yakkha  employs another noun for this strategy, namely \emph{laŋ}, with the lexical meaning \rede{foot}. It is, however, not a \isi{nominalizer};  \emph{laŋ} cannot be used to nominalize propositions, as in \rede{the place where he cut the meat}. In compounds, \emph{laŋ} designates the area surrounding an object or characterized by it, as e.g., in \emph{khibulaŋ} \rede{area around walnut tree} or \emph{tonalaŋ} \rede{uphill area}. One also finds lexicalized instances, such as in \Next, or metaphorical extensions, as in \emph{pheksaŋlaŋ} \rede{malicious wizard} (left-foot/left-side). It does not come as surprise that \isi{toponyms} contain this marker, e.g., \emph{lokphalaŋ} \rede{grove of lokpha bamboo (a huge kind of bamboo)}. However, the \isi{number} of examples in the existing data do not allow conclusions about the productivity of \emph{laŋ}. 

\exg. maŋcwalaŋ=be khy-a-ŋ\\
water\_tap{\sc =loc} go{\sc -pst-1sg}\\
\rede{I went to the public water tap.} \source{40\_leg\_08.048} 

This \isi{compounding} strategy has developed from a relational noun construction (see \Next and §\ref{postpos} below). The relational noun \emph{laŋ} locates an object (the {\sc figure}) next to the lower part of another object (the {\sc ground}). 
...
Many \isi{botanical terms} end in \emph{siŋ} for \rede{tree} or in \emph{phuŋ} for \rede{flower}, e.g., \emph{likliŋphuŋ} \rede{mugwort} and \emph{kekpusiŋ} \rede{bull oak}. Above, in §\ref{lex-noun-1}, nouns in \emph{-wa} were discussed as fossilized nominalizations. A homophonouns morpheme with the etymological meaning of \rede{water} is found in 14 lexemes referring to \isi{liquids}, such as  \emph{cuwa} \rede{beer}, \emph{naŋwa} \rede{glacier} (snow-water), \emph{casakwa} \rede{water in which uncooked rice has been washed} (rice-water), \emph{lithuʔwa} \rede{sperm} and \emph{mikwa} \rede{tear} (eye-water). 
...
\subsection{Core \isi{case} markers (Group I)}\label{case}
...
Yakkha shows a typologically common \isi{nominative}/\isi{ergative} syncretism: transitive subjects that are represented by a first or second person pronoun always appear unmarked (cf. §\ref{case-erg}).
...
\subsubsection{The \isi{ergative} \emph{=ŋa}}\label{case-erg}
...
In Yakkha, first or second person reference can also be instantiated by full nouns instead of pronouns, which is unusual from the perspective of Indo-European languages. One may have a sentence with first or second person verbal \isi{person marking}, but the structural position of the pronoun is occupied by a noun, as shown in \Next.\footnote{Flexible agreement is discussed in §\ref{flex-agr}. On the principles behind agreement in \isi{Tibeto-Burman} see  \citet{Bickel2000On-the-syntax}.} In such participant configurations, there is overt \isi{ergative} marking on the noun. To make a long story short, the differential agent marking is mainly determined  by word class, but also by reference.
...
The instrumental also indicates temporal reference (see \Next). On a side note,  it is very likely that the adverbial clause linkage markers  \emph{-saŋ} and \emph{=niŋ(a)} (both marking cotemporality)  are based on the \isi{ergative}/instrumental \isi{case} etymologically.
...
\subsubsection{The \isi{genitive} \emph{=ka} }\label{case-gen}		
The \isi{genitive} \isi{case} is marked by the suffix \emph{=ka} (mostly realized as [ga] as  result of the \isi{voicing} rule, see §\ref{voicing}). It is used for possessive constructions, linking a possessor to a head noun (see \Next). As mentioned in §\ref{poss} on \isi{possessive pronouns}, the possessee may be inflected by a possessive prefix, as in \Next[b] and \Next[c]. The possessive inflection may occur in addition to a genitive-marked possessor, or may replace it, as in \Next[c].
...
Relational nouns functioning as spatial adpositions also require the \isi{genitive}, illustrated by \Next . They are used in a possessive construction to which a \isi{locative} must be added (see \Next[b]; cf. also §\ref{postpos}). 
...
\subsubsection{The \isi{locative} \emph{=pe}}\label{case-loc}
...
The basic \isi{locative} construction \cite[15]{Levinsonetal2006_Grammars}, the answer to the question \rede{Where is F?} is a copular construction with \emph{wama} (with the suppletive nonpast stems \emph{waiʔ}, \emph{wɛʔ}, \emph{wei}) \rede{be, exist} (see \Next[a]). The same construction (with different \isi{information structure}) is generally used to introduce topics in the beginning of narratives (see \Next[b] and \Next[c]).
...
Destinations of \isi{motion verbs} and verbs of caused \isi{motion} are generally marked by the \isi{locative}, illustrated by \Next. As explained above in §\ref{case-nom}, in certain scenarios the \isi{locative} marking on the destinations of \isi{motion verbs} can be omitted. 
...
It is not surprising to find the \isi{locative} marking extended to temporal reference. However, the more frequent marker in this function is the instrumental \emph{=ŋa}. The \isi{locative} in \Next might well be a \ili{Nepali} calque, since, except for \emph{na}, all words in \Next are \ili{Nepali} loans. 
...
The medium of \isi{motion} and the technical medium of communication can also be marked by  the \isi{ablative},  in parallel to the functions of the \ili{Nepali} \isi{ablative} \emph{bāṭa}. 
...
\rede{From the lowlands (local lowlands, not the Tarai), some people.}\source{36\_cvs\_06.465}
 ...
 In some interrogative words and adverbs one can still see that they were composed of some root and an older \isi{ablative} marker, e.g.,  in \emph{nhaŋ} \rede{from here/and then} and \emph{heʔnaŋ \ti heʔnhaŋ} \rede{where from}.\footnote{Both forms  \emph{heʔnaŋ} and \emph{heʔnhaŋ} are equally acceptable to the speakers, and semantic differences could not be detected.} 
...
The \isi{ablative} is generally not used for temporal reference. There is a postposition \emph{nhaŋto} that covers this function (cf. §\ref{postpos} below).
 

\subsubsection{The \isi{comitative}  \emph{=nuŋ}}\label{case-com}

The \isi{comitative} marker  \emph{=nuŋ} is cognate to \ili{Limbu} \emph{-nu},  \ili{Thulung} \emph{-nuŋ} \citep[81]{Ebert1994The-structure}, Wambule \emph{-no} \citep[157]{Opgenort2004A-Grammar}, \ili{Bantawa} \emph{-nin} \citep[91]{Doornenbal2009A-grammar}, \ili{Chintang} \emph{-nɨŋ} \citep[80]{Schikowski2012_Morphology}. It can be used as a nominal coordinator, functionally similar to English \emph{and}  (symmetrical, with nouns of the same status, as defined in  \citet[3]{Haspelmath2004_overview}). An example is given in \Next[a], a story title of the commonly found pattern \rede{X and Y}. Thus, by its very nature, this \isi{case} marker can be found inside noun phrases, coordinating two nominal heads. The other \isi{case} markers attach to the coordinate structure as a whole (see \Next[b]). The marker is phonologically bound to the first component of the coordinate structure.
...
Some frames of verbal \isi{argument realization} (both intransitive and transitive) require the \isi{comitative} on their arguments, such as \emph{cekma} \rede{talk}, \emph{toŋma} \rede{fit/agree/belong to}, \emph{kisiʔma} \rede{be afraid}, \emph{nakma} \rede{ask} and \emph{incama} \rede{buy (from)} (see \Next).		
...
The \isi{comitative} also plays a role in the derivation of some adverbs, as shown in \Next (cf.  §\ref{adv}). Furthermore, it is also found in clause linkage (cf.  §\ref{com-cl}).
...
\subsection{Further \isi{case} markers (Group II)}\label{postpos}
  
The markers of Group II are quite heterogenous; they do not define a class as such. They can appear bound to their host or independently, i.e., stressed like a separate word. Their phonological weight is also greater than that of the markers of Group I; all of them are at least disyllabic.  The \isi{case} markers of Group II  have a greater flexibility with regard to hosts they can select. Not only nominals are possible, but also adverbials. Some markers of Group II  are not attested with nominal complements at all, like \emph{khaʔla} \rede{towards}.  Furthermore, a \isi{number} of the markers of Group II have hybrid word class status; they can also be used as adverbs. Some markers were borrowed into the language from \ili{Nepali}, like \emph{samma} \rede{until} or \emph{anusar} \rede{according to}.  Table \ref{table-postpos} provides a summary of all Group II markers and their functions, described in detail in the following sections.
...
\midrule
\emph{bahek} [NEP]& \isi{exclusive}, \rede{apart from}\\
\emph{samma} [NEP]&terminative, \rede{until, towards}\\
\emph{anusar} [NEP]&\rede{according to}\\
\emph{lagi} [NEP]&\isi{benefactive}, \rede{for}\\
\lspbottomrule
\end{tabular} 
...
\subsubsection{The temporal \isi{ablative} marker \emph{nhaŋto} }
...
\subsubsection{The \isi{equative } marker for size \emph{hiŋ}}
...
Table \ref{relnoun} provides a summary and the original \isi{lexical nouns} that are the bases for each relational noun. In \Next, the relational noun is reduplicated, since the relation described is not one of location at the riverside, but one of movement along the river.
...
Furthermore, there are spatial adpositions,  presenting an orientation system that is based on the uphill/downhill distinction. They are treated in §\ref{geomorph-postp}, together with the other word classes that are based on this topography-based system.
...
\subsection{Other modifiers: \isi{adjectives}, \isi{numerals}, \isi{quantifiers}, demonstratives}\label{str-np-mod}
...
\subsection{Relative clauses}\label{str-np-rc}
 ...
 \rede{the steam of deep-fried bread, fried chili and boiled yams}\footnote{The lexeme \emph{macchi} is a loan from the \ili{Nepali} source word \emph{marej} \rede{pepper}. In Yakkha, it refers to chili peppers, but also to hot pickles and sauces.} \source{40\_leg\_08.046 }
...
Some nouns take clausal complements (see §\ref{noun-compl}).
