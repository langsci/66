...
\item Only inflectional suffixes, but not phrasal clitics,\footnote{Placing clitics between the verbal stems is indeed possible, for instance in \ili{Chintang} \citep{Bickeletal2007Free}.} clause-level particles or clause linkage markers attach to the V.lex. 
...
\lsptoprule
\emph{-piʔ}&(a) \isi{benefactive}/malefactive& \rede{give} & 3\\
 &(b) affected participants& &\\
(\emph{-piʔ} \ti  \emph{-diʔ})&(c) telic, \isi{completive} (intrans.)& &1\\
\emph{-i \ti -ni}&\isi{completive} (trans.)&(only V2)&2\\
\emph{-ca}&(a) reflexive, &\rede{eat}&2\\
 &(b) auto\isi{benefactive},& &(but often \\
 &(c) middle (intentional)& &detransitivized)\\
\emph{-siʔ}&middle (\isi{unintentional})&(only V2)&1\\
\emph{-kheʔ}&(a) \isi{motion} away&\rede{go}&1\\  
&(b) telic (S/P arguments)&&\\ 
\emph{-ra} (*\emph{ta})&\isi{motion} towards &\rede{come (from further away)}&1\\ 
\emph{-raʔ} (*\emph{taʔ})&caused \isi{motion} towards &\rede{bring (from further away)}&3\\%27.11 example
\emph{-uks} & \isi{motion} down towards&\rede{come down}& 1\\
\emph{-ukt} &caused \isi{motion} down towards&\rede{bring down}& 3\\
\emph{-ap}&   \isi{motion} towards& \rede{come (from close nearby)}&1\\
\emph{-apt}&  caused \isi{motion} towards& \rede{bring (from close nearby)}&3\\
\emph{-ris} (*\emph{tis})&caused \isi{motion} to a distant goal & \rede{invest, place} &3\\
\emph{-bhes}&caused horizontal \isi{motion} &\rede{send, bring here}&3\\
\emph{-end \ti -neN}& (a) caused \isi{motion} downwards,&\rede{insert, apply}&3\\
&(b) accidental actions, regret&&\\
\emph{-ket}&caused \isi{motion} up and towards&\rede{bring up}&3\\
\emph{-haks} \ti \emph{-nhaŋ}& (a) caused \isi{motion} up and away &\rede{send}&3\\
&(b) irreversible caused change-of-state&  &\\
\emph{-khet \ti -het}&(a) caused \isi{motion} away &\rede{carry off}&3\\
&(b) telic, excessiveness (transitive)&&\\
\emph{-a \ti -na}&do X and leave object behind&(only V2)&3\\
\emph{-nes}&\isi{continuative}&\rede{lay}&3\\
\emph{-nuŋ}&\isi{continuative}&(probably \emph{yuŋ} \rede{sit})&1\\
\emph{-bhoks}&\isi{punctual}, sudden&\rede{split}&2\\
	& events& &\\
\emph{-heks}&(a) \isi{immediate prospective}&\rede{cut}&2\\
 &(b) do separately& & \\
\emph{-ghond}&\isi{spatially distributed events} &\rede{roam}&2\\
...
\subsection{The V2 \emph{-piʔ} (\isi{benefactive}, affected participants)}\label{V2-give}%\rede{give} 
...
The V2 \emph{-piʔ  \ti -diʔ} is also found in lexicalized CPs, contributing \isi{transitivity} information. Certain lexical stems never occur independently; they have to be in a \isi{complex predicate} construction. Their \isi{valency} is not specified; different  V2s may combine with them to specify their \isi{transitivity}. There are two  corresponding sets of predicates: one intransitive, built by adding the V2 \emph{-piʔ  \ti -diʔ} \rede{give}, and one transitive, built by adding the marker \emph{-i \ti -ni}  (see §\ref{V2-compl} below) to the lexical verb. This alternation is illustrated by \Next and \NNext. The alternations do not always have  the same direction in terms of \isi{argument structure}. In \Next, the intransitive subject corresponds to the P argument in the corresponding transitive predicate, while in \NNext, it corresponds to the A argrument. Table \ref{pi-ni} provides further examples of this symmetrical alternation.
...
\subsection{The V2 \emph{-ca} (reflexive, middle, auto\isi{benefactive})}\label{V2-eat}%\rede{eat} 
...
\subsection{The quasi-V2 \emph{-siʔ} (middle, \isi{unintentional})}\label{V2-mddl}
...
\subsection{The V2 \emph{-kheʔ} (\isi{motion} away, \isi{telicity})}\label{V2-go}% \rede{go} 
...
\subsection{The V2 \emph{-ra} (\isi{motion} towards)}\label{V2-comeneut} %\rede{come}}
...
\subsection{The V2 \emph{-raʔ}  (caused \isi{motion} towards)}\label{V2-bringneut}%\rede{bring}
...
\subsection{The V2 \emph{-uks} (\isi{motion} down towards)}\label{V2-comedown}%\rede{come down} 
...
\subsection{The V2 \emph{-ukt}  (caused \isi{motion} down towards)}\label{V2-bringdown}%\rede{bring down}
...
\subsection{The V2 \emph{-ap} (\isi{motion} towards, from close nearby)} \label{V2-come}
...
\subsection{The V2 \emph{-ris} (caused \isi{motion} to a distant goal)}\label{V2-place}%\rede{place, invest} 
...
\subsection{The V2 \emph{-bhes} (caused horizontal \isi{motion} towards)} \label{V2-bhes}%\rede{bring horizontally}
...
\subsection{The V2 \emph{-end}  (caused \isi{motion} downwards)} \label{V2-insert}%\rede{apply, insert}
...
\subsection{The V2 \emph{-ket} (caused \isi{motion} up and towards)}\label{V2-bringup}%\rede{bring up} 
...
\subsection{The V2 \emph{-haks} (caused \isi{motion} away, irreversibility)}\label{V2-send}%\rede{send} 
...
\subsection{The V2 \emph{-khet} (caused \isi{motion} along with A)}\label{V2-carryoff}%\rede{carry off}
...
\subsection{The V2 \emph{-nes}  (\isi{continuative})}\label{V2-lay}%\rede{lay}
...
\subsection{The V2 \emph{-nuŋ}  (\isi{continuative})}\label{V2-nung}%\rede{sit}
...
\subsection{The V2 \emph{-bhoks}  (\isi{punctual}, sudden events)}\label{V2-split}%\rede{split}
...
\subsection{The V2 \emph{-heks}  (\isi{immediate prospective},  do separately)}\label{V2-cut}%\rede{cut, saw}
...
This V2 has a second meaning, translatable as \rede{do separately}. The corresponding construction in \ili{Nepali} is \emph{[{\sc V.stem}]-dai garnu}. This usage of \emph{-heks} is often found in commands, for instance when the speaker encourages the hearer to start or go on with some activity while the speaker leaves the speech situation (see \Next). 
...
\subsection{The V2 \emph{-ghond} (\isi{spatially distributed events})}\label{V2-roam}%\rede{roam} 
