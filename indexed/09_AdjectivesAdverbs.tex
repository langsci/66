...
\midrule
\emph{cicaŋgalik{\sc -redup}} &\rede{tumbling, overturning}&\emph{caks}  \newline \rede{overturn}\\%[1.5em] %\newline (in somersaults, bulky objects)
\emph{hiklik} &\rede{turned around, upside down}&\emph{hiks} \newline \rede{turn}\\%[1.5em] 
%\emph{hiŋ} \rede{survive}&\emph{uhiŋgilik} &\rede{alive} \\[1.5em] 
\emph{iplik{\sc -redup}} &\rede{properly [twisted]}&\emph{ipt} \newline \rede{twist, wring}\\%[1.5em] 
\emph{kakkulik{\sc -redup}} &\rede{tumbling or {rolling down}}&\emph{kaks}  \newline \rede{fall}\\%[1.5em] % \newline (round objects, smooth movement) 
\emph{pektuŋgulik} &\rede{[folded] properly, many times}&\emph{pekt}  \newline \rede{fold}\\%[1.5em] 
\emph{phoplek} &\rede{[pouring out]  at once} &\emph{phopt} \newline  \rede{spill, pour}\\%[1.5em] 
\emph{siklik} &\rede{[dying] at once}&\emph{si} \newline  \rede{die}\\%[1.5em] 
\emph{sontrik} &\rede{[manner of] sliding, falling}&\emph{sos} \newline  \rede{lie slanted}\\%[1.5em] 
\emph{wakurik} &\rede{bent, crooked}&\emph{wakt} \newline  \rede{bend forcefully} \\%[1.5em] 
\emph{hobrek} &\rede{[rotten] completely}&\emph{hop} \newline  \rede{rot} \\%[1.5em] 
\lspbottomrule
\end{tabular}
}
