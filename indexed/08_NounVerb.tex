...
Note that if \emph{wepma} in  \Last[b] were a regular verbal argument, an instrumental \isi{case} would be expected, since it is an effector with respect to the verbal meaning. And indeed, some noun-verb predicates require an instrumental or an \isi{ergative} \isi{case} on the noun (see \Next).\footnote{Yakkha has an instrumental/\isi{ergative} syncretism. Therefore, in intransitive predicates \emph{=ŋa} is interpreted as instrumental; in transitive predicates it is interpreted as \isi{ergative}.} 
...
Class (b) differs from class (a) in that the \isi{psych-noun} triggers object agreement, invariably third person and in some cases, third person plural (see \Next[b]). No stimulus is expressed in class (b). This class has the highest \isi{number} of members. 
...
All frames of \isi{argument realization} with examples are provided in Tables \ref{tab-exp1} and \ref{tab-exp2}.\footnote{Stems in square brackets in the tables were not found as independent words beyond their use in these collocations.}
...
but some combinations were judged better than others (compare \Next[a] with \Next[b]). Compare also the impossible additive focus particle \emph{=ca} in \Next[a] with the \isi{restrictive focus} particle \emph{=se} and the contrastive particle \emph{=le} in \NNext. Overtly expressed \isi{experiencer} arguments  may naturally also host topic and focus particles, just like any other constituent can. This is shown e.g. by \Next[b] and by \NNext[c] and (d).
