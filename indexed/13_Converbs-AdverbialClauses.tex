...
\section{The \isi{supine converb} \emph{-se}}\label{sup}
...
The above mentioned coreference constraint is strictly semantic, applying irrespective of the question of \isi{argument realization}. In \Next, the subject is a non-canonically marked \isi{experiencer}, realized as the possessor of the laziness (cf. §\ref{exp}). As long as the argument is highest-ranking in terms of semantic roles, i.e., the  most agent-like argument, it qualifies for the coreference constraint of the \isi{simultaneous converb}.\footnote{The \isi{reduplication} of the \isi{converb}, as in this particular example, signifies either ongoing or iterative events.}
...
It should also be mentioned that this converbal structure is  used for a \isi{periphrastic continuative} \isi{aspect} construction that is probably calqued upon a similar structure in \ili{Nepali}, too (see \Next, and §\ref{tense}).
...
\section{The \isi{negative converb} \emph{meN...le}}\label{menle}
...
Constituents within the clause may carry focus markers. Consider example \Next, with the interrogative pronoun being focussed on by the additive focus particle \emph{=ca}, that results in the exhaustive \isi{negation} “any” in combination with the \isi{negative converb}. 
...
There are no constraints on coreference. In \Next[a], the referents of the  S and A arguments, respectively, are not identical, while  they are in \Next[b]. The example in \Next[a] could alternatively be expressed by the \isi{negative converb} construction discussed  in §\ref{menle}. 
...
The clause is not only reduced with regard to the \isi{tense}/\isi{mood} distinction; sentence-level markers and the main clause nominalization are not possible either. 
...
The scope properties are similar to those of the sequential clauses. With regard  to \isi{negation}, overall and main clause scope are possible. Overall scope is shown in \Next[a], and main clause scope in \Next[b], which acquires a causal reading in addition to the basic cotemporal one. 
