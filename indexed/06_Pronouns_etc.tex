...
This chapter describes the elements that can be found in the \isi{noun phrase},  mo\-di\-fying or replacing a head noun. It is structured as follows: \sectref{pers-pron} deals with the \isi{personal pronouns}, \sectref{poss-pron} discusses the \isi{possessive pronouns}, and \sectref{dem-pron} is concerned with the \isi{demonstratives}. \sectref{sec-indef} shows how indefinite reference is expressed, \sectref{sec-quant} deals with \isi{numerals} and other quantifying elements. \sectref{interr} then focuses on interrogative forms, including non-nominal interrogatives. 
  
\section{Personal pronouns}\label{pers-pron}

Yakkha \isi{personal pronouns} are used to refer to persons, typically those participants to whom reference has already been made in discourse. Personal pronouns can take the structural position of a \isi{noun phrase} (of any participant role) or they can function as  heads of noun phrases, although the possibilities to be modified are restricted: relative clauses and \isi{demonstratives} are not possible, for instance. Possible modifiers are \isi{quantifiers} and \isi{numerals}, but they follow the pronominal head, in contrast to noun phrases with nominal heads, which are mostly head-final. Pronouns, like noun phrases in general, are not obligatory, and they are frequently dropped in Yakkha.

The pronouns distinguish person and \isi{number}. Clusivity, which is found  in \isi{possessive pronouns}, \isi{possessive prefixes} and in the verbal inflection,  does not play a role in the \isi{personal pronouns} (compare \Next[a] and \Next[b]). An overview of the \isi{personal pronouns} is provided together with the \isi{possessive pronouns} in \tabref{poss} below. The first and second person pronouns distinguish singular, dual and plural \isi{number}. The morpheme \emph{-ci} indicates dual number in the first and second person pronouns, while \emph{-ni} indicates the plural. In the third person, \emph{-ci} simply has a nonsingular meaning.\footnote{Note that in contrast to the pronominal paradigm, the verbal inflection distinguishes dual \isi{number} also in the third \isi{person} (cf.  \sectref{verb-infl}).} 
...
The \isi{possessive pronouns} modify a head noun, indicating the possessor of the thing that is referred to by the noun (see \Next[a]). Since the head noun can be omitted when its reference has been established already, the possessive pronoun can also be the sole element in a phrase (see \Next[b]). 

The \isi{possessive pronouns}  resemble the \isi{personal pronouns} to some degree, but they are sufficiently different and irregular so that they establish a separate paradigm. Except for the third person nonsingular form, the roots all look slightly different from the corresponding \isi{personal pronouns}.  They all host the \isi{genitive} en\isi{clitic} \emph{=ga}, though. The \isi{possessive pronouns} distinguish \isi{number} and person, including \isi{clusivity}, a category that is  absent from the personal pronoun paradigm.  The \isi{inclusive} forms have no parallel in the \isi{personal pronouns}. \tabref{poss} provides an overview of personal and \isi{possessive pronouns} and \isi{possessive prefixes}. The capital /N/ stands for an unspecified nasal that assimilates to the following consonant  in place of articulation.
...
\lspbottomrule
\end{tabular}
\caption{Personal and \isi{possessive pronouns}, possessive inflection}\label{poss}
\end{centering}
}
\end{table}
...
Relationships of possession can also be expressed by attaching a possessive prefix to the head noun, which refers to the possessee. The prefixes index the \isi{number} and person of the possessor. Their form is similar to that of the \isi{possessive pronouns}, which suggests that they have developed out of them. The \isi{nasals} in the {\sc 1pl.incl} prefix \emph{eN-} and in the {\sc 2sg} prefix \emph{N-} assimilate in place of articulation to the first consonant of their nominal host (see \Next). The third person singular prefix \emph{u-} has the allomorph \emph{o-} before stems containing /e/ or /o/. The possessees can also be nouns referring to sensations, as in \Next[a]. 

It is \isi{information structure} that determines whether a pronoun or a prefix is used. If the possessive relationship is focused on, the pronoun has to be used.
...
Certain nouns nearly always  appear with \isi{possessive prefixes}, even when no clear possessor has been  mentioned in the preceding discourse. The semantic domains which are relevant for obligatory possession are consanguineal \isi{kinship}, spatial relations (\isi{relational nouns}),  body parts and other part-whole relations that are not body parts in the strict sense, such as \emph{otheklup} \rede{half} or \emph{ochon} \rede{splinter}. So far, 118 obligatorily possessed nouns could be found, which makes up roughly 9\% of the nominal lexicon.\footnote{In  \citet[242]{Bickeletal2005_Obligatory} on obligatorily possessed nouns, this phenomenon is defined as “words for which an inflectional category of possession is obligatorily present”. In the current Yakkha data at least some exceptions can be found, so that I conclude that obligatory possession is a rather gradual phenomenon  in Yakkha. More data would be necessary in order to explain apparent exceptions and thus to paint a clearer picture of obligatorily possessed nouns in Yakkha.} Some of the  obligatorily possessed nouns are listed in Tables \tabref{inalien1} and \tabref{inalien2}. Since obligatory possession is also found in the expression of spatial relations, several adverbs and \isi{relational nouns} originate in obligatorily possessed nouns (cf. \sectref{adv}).
...
\largerpage
While the default option for kin terms is the first person prefix, for the other obligatorily possessed nouns it is the third person singular, as for instance in \emph{u-ʈiŋ} \rede{thorn}. We find some lexicalized instances here as well, for instance \emph{usa} \rede{fruit}, stressed on the first \isi{syllable} and lexicalized from the more general noun \emph{sa}, translating as \rede{flesh, meat} and \rede{fruit flesh}. Another instance is \emph{uwa} \rede{nectar, honey, (any) liquid}, also stressed on the first \isi{syllable}, with the original meaning \rede{water} or, more generally, \rede{liquid}. 

Other Kiranti languages also have obligatory possessive marking. Camling also has obligatory possessive marking on inherently \isi{relational nouns} \citep[41]{Ebert1997Camling}. Similarly, \citet[98--100]{Doornenbal2009A-grammar} lists  classes of nouns that necessarily occur with possessive marking. In her grammar of \ili{Thulung}, \citet[72]{Lahaussois2002Thulung} mentions that an otherwise rare combination of possessive prefix and \isi{genitive} marking is frequently found with inalienably possessed nouns such as nouns from the domains of \isi{kinship} and body parts.
...
The functional core of \isi{demonstratives} is \isi{deixis}. Demonstratives (just like pronouns and temporal adverbs such as \rede{tomorrow}) are deictic; their reference depends on a center that is established in the  particular utterance context and that may thus  change with that particular utterance context \citep{Buehler1934_Sprachtheorie, Fillmore1997_Deixis}. The point of reference is typically, but not necessarily, the speaker. 


There are two sets of \isi{demonstratives} in Yakkha, one set based on proximity and distance to the deictic center (spatial as well as anaphoric, see \sectref{dem-pron-1}) and one set based on the inclination of the landscape, called \emph{geomorphic} in \citet{Bickel1997Spatial}. The latter are treated separately in \sectref{dem-pron-2} on the topography-based orientation system. The roots of the former set are pronominal, but they can become adverbial via derivations (see \sectref{dem-set1rel}). 


\subsection{Proximal, distal and anaphoric deixis}\label{dem-pron-1}

\tabref{dem-tab} shows the forms expressing the three-fold distinction between proximal, distal and anaphoric \isi{demonstratives}. The proximal forms are used to refer to objects or people that are close to the speaker and can be touched or pointed at, while the distal forms are used for objects or people further away as well as for referents that are not present in the speech situation. Narratives mostly use the distal forms, except in direct quotations. The anaphoric \isi{demonstratives} are used to take up  reference to some participant that had  already been activated at a previous time in discourse; it is best translated as  \rede{that very (person/thing/event)}. The members of this set of \isi{demonstratives} are also found in \isi{correlative clauses} (see \sectref{correlative}). Demonstratives can be used adnominally (i.e., modifying a head noun) and pronominally (i.e., replacing a \isi{noun phrase}) in Yakkha. Furthermore, \isi{demonstratives} may replace \isi{personal pronouns} in the third person, as the use of \isi{personal pronouns} is considered somewhat rude.
...
\largerpage
Let us first take a look at the proximal-distal distinction.  In example \Next, the \isi{demonstratives} are used in attributive function. The \isi{number} distinction is encoded by the base forms for \isi{proximal deixis} \emph{na} (singular) and \emph{kha} (nonsingular and non-countable reference).\footnote{The distinction between singular on the one hand and nonsingular/non-countable on the other hand is fundamental and robust in Yakkha, found not only in the  \isi{demonstratives} but also in nominalizations and in verbal agreement.}  Distal \isi{deixis} is expressed by adding either a prefix \emph{nna} or just a homorganic nasal to these roots (not segmented in the glosses).\footnote{In Belhare \citep[548]{Bickel2003Belhare}, the lexeme corresponding to \emph{nna} is \emph{ina}. The same sound correspondence (between nasal prefix and prefix \emph{i-}) is found between the Tumok and  the Kharang \isi{dialects} of Yakkha.} No semantic difference between \emph{nnakha} and \emph{ŋkha} could be determined, and the latter seems like a contracted form of the former. In terms of \isi{stress assignment}, these \isi{demonstratives} may cliticize phonologically when they are used attributively, but they are generally able to carry their own stress. They naturally  carry stress when they occur on their own, e.g., \emph{khaci} \rede{these}.
...
As example \Last shows, all \isi{demonstratives} can appear as nominal modifiers (see also \Next). The non-countable reference of \emph{kha} can be illustrated by the difference between \emph{toŋba} \rede{beer served in a small barrel and drunk with a pipe} and \emph{cuwa} \rede{beer}. While the first has countable reference, the second is treated as a substance and hence has non-countable reference. The demonstrative \emph{kha} may thus refer to nonsingular instances of \isi{count nouns} (see \Next[b]) or to \isi{mass nouns}  (see \Next[c]). This distinction of \isi{number} and countability is also reflected in the sentence-final nominalizers in these examples, which are etymologically related to the \isi{demonstratives} (discussed at length in \sectref{nmlz-uni}).
...
The \isi{demonstratives} may also head noun phrases, hosting the phrasal morphology and triggering agreement (see \Next). They are more restricted than nominal heads of noun phrases, as they cannot take adnominal modifiers.  
...
The anaphoric \isi{demonstratives} identify referents that have already been activated in discourse, and are taken up again, as in \Next. The speaker introduces her narrative with the fact that she has seen a film. Then, the listener makes a joke, distracting away from the film (not included in the example).  The speaker re-introduces the topic with \emph{honna}.
...
In \Next, a written narrative, the protagonist goes fishing to surprise his sick father but he loses the net in the strong current of the river. After losing the net, he says:
...
The singular form  \emph{na} could be etymologically related to a topic particle of the same form, as it is still found in Belhare or \ili{Puma}, for instance \citep[559]{Bickel2003Belhare, Bickeletal2009Puma}. Furthermore, the \isi{demonstratives} \emph{na} and \emph{kha} have  developed into the nominalizers \emph{=na} and \emph{=ha} which show exactly the same distribution with regard to \isi{number} and the count/mass distinction as the \isi{demonstratives} (cf. \sectref{nmlz-uni}). On a final note, clause-initial coordinators like \emph{nhaŋ}, \emph{nnhaŋ}, \emph{khoŋ} and \emph{ŋkhoŋ} (all paraphrasable with \rede{and then} or \rede{afterwards}) are \isi{demonstratives} with \isi{ablative} marking historically.


\subsection{Demonstrative adverbs and quantifiers}\label{dem-set1rel}

The proximal-distal-anaphoric distinction is also present in a set of \isi{demonstrative adverbs} and \isi{quantifiers}, as given in \tabref{pr-di-an-adv}. Example \Next some more instances of anaphoric \isi{demonstrative adverbs} based on the root \emph{hon}. The sentence in \Next[a] is uttered at the end of a narrative, and the adverbs refer to the content and number of the events just related.\footnote{Quantifying expressions (both for amount and size) are the topic of \sectref{sec-quant} below.}  In \Next[b], \emph{hoŋkhaʔniŋ} refers to the time at which the events took place (specified in a previous sentence), and in \Next[c], \emph{honnhe} refers to the place just mentioned in the conversation.
\largerpage[2]
...
&\rede{here}	&\rede{there}	&\rede{where mentioned before}\\ \midrule
{\sc time}&		\emph{khaʔniŋ} 	&	\emph{ŋkhaʔniŋ}  \ti \emph{nnakhaʔniŋ}	& \emph{hoŋkhaʔniŋ}\\
& \rede{this time, now}&\rede{that time, then}	&\rede{right at that time}\\\midrule
{\sc manner} 		&	\emph{khaʔla} 	&	\emph{ŋkhaʔla} \ti \emph{nnakhaʔla}& \emph{hoŋkhaʔla} \\
&\rede{like this}	&\rede{like that}	&\rede{like mentioned \mbox{before}}\\\midrule
{\sc amount}/&		\emph{khiŋ}  	&	\emph{ŋkhiŋ} \ti \emph{nnakhiŋ}	& \emph{hoŋkhiŋ} \\
{\sc size/}&\rede{this much}&\rede{that much}&`as much as  \mbox{mentioned} before'\\
{\sc degree} &\rede{this big}&\rede{that big}&\rede{as big as \mbox{mentioned} before}\\
\lspbottomrule
\end{tabular}
...
Yakkha does not have a morphologically distinct class of indefinite pronouns; all pronouns and \isi{demonstratives} are definite. There are, however,  several strategies to convey indefinite reference, including the use of simple nouns. Occasionally, the numeral \emph{eko} \rede{one} is also used for this purpose. In example \Next[a], \emph{eko} refers to an object in a future and hence irrealis statement; in \Next[b], \emph{eko} refers to a specific (known to the speakers), but still indefinite \isi{person} (not determined in a way that the hearer can identify the referent).
...
Exhaustive reference, i.e., including all imaginable referents in a given context, is expressed by attaching the \isi{additive focus} particle \emph{=ca} to an interrogative pronoun (see \Next). This works with affirmative and with negated statements; in the latter \isi{case}, it has the effect of exhaustive \isi{negation} (see \Next[c]).
...
Another strategy to express indefinite reference is to use an interrogative pronoun and to reduplicate  the fully inflected verb (see \Next). Additionally, the interrogative phrase may host a \isi{topic marker} \emph{=ko}, which is not possible in interrogative utterances, since the inherent focus of interrogative phrases rules out topic marking on them. Both strategies help to disambiguate indefinite statements and interrogative utterances.
...
Yakkha has several \isi{quantifiers} to indicate the amount, size,  \isi{degree} or intensity of the concepts expressed by nouns, \isi{adjectives} or verbs. They are listed in \tabref{quant}, with the  word classes with which they combine. The form  \emph{maŋpha} \rede{much/very} is special insofar as it may also express  the \isi{degree} of another quantifier, such as in \emph{maŋpha pyak} \rede{really much}. The table also includes deictic \isi{quantifiers} and \isi{degree} words.
...
\rede{Please make the sky a little cloudy.}  \source{37\_nrr\_07.100}
 ...
 Furthermore, \emph{miyaŋ} is also found with \isi{adjectives} and  adverbs (see \Next).
...
The quantifier \emph{pyak} is used with count and \isi{mass nouns}. It has an intensifying function when it is combined with verbs and adverbs or \isi{adjectives}. It indicates a large amount or a high \isi{degree} of whatever is expressed by the head that it modifies. Thus, it can be rendered with  English \rede{much}, \rede{many} and \rede{very}. Examples are provided below in \Next for the nominal domain and in \NNext for verbal and adverbal/adjectival uses. In \NNext[a], \emph{pyak} is further emphasized by the deictic \isi{degree} particle \emph{khiŋ}, yielding the \isi{exclamative} \rede{how much!}.
...
The exhaustive quantifier \emph{ghak} \rede{all, whole} can refer to an exhaustive \isi{number} or amount, as in  \Next[a], or to a complete unit, as in \Next[b] and  \Next[c]. The potential ambiguity is resolved by the verbal \isi{number} agreement, which has to be plural in the exhaustive reading.

\ex.\ag.ghak limbu          m-bog-a-ma-ci=hoŋ, ...\\
all \ili{Limbu}\_\isi{person} {\sc 3pl-}get\_up{\sc -pst-prf-nsg=seq}\\
...
The deictic quantifier \emph{khiŋ} has to be interpreted with respect to the utterance context; it can  refer to  amount or size. In most cases, its use is accompanied by  gestures that indicate the size or the amount of some entity. Occasionally,  the nominal \isi{comitative} can be found attached to \emph{khiŋ}  (see \Next[c]).

\ex.\ag.khiŋ     tukkhi ŋ-aŋd-u ...\\
this\_much pain {\sc 3pl.A-}endure{\sc -3.P[pst]}\\
\rede{They endured so many troubles ...}  \source{14\_nrr\_02.07}
...
Like the \isi{demonstratives} described in \sectref{dem-pron}, \emph{ŋkhiŋ} may express distal reference, i.e., \rede{that much} (compare \Next[a] and \Next[b]). In \Next[b], instead of indicating the size with his own hands, the speaker points to a piece of wood lying nearby. The distal reference is also used in general statements, as in \Next[c].

\ex.\ag.puchak khiŋ=na sa=na!\\
snake this\_much{\sc =nmlz.sg} {\sc cop.pst[3sg]=nmlz.sg}\\
\rede{The snake was this bigǃ} (The speaker is showing with her own hands how big it was.)
\bg.puchak ŋkhiŋ=na sa=na!\\
snake this\_much{\sc =nmlz.sg} {\sc cop.pst[3sg]=nmlz.sg}\\
\rede{The snake was that bigǃ} (The speaker is pointing to a piece of wood lying nearby.)
...
Anaphoric \isi{deixis} is possible as well, using \emph{hoŋkhiŋ}. The sentence in \Next follows a long enumeration of particular things the protagonist had to do, and \emph{hoŋkhiŋ} refers back to them.
...
The inherited \isi{Tibeto-Burman} \isi{numerals} have largely been lost in Kiranti \citep{Ebert1994The-structure}. In Yakkha only the \isi{numerals} \emph{i} \rede{one}, \emph{hiC} \rede{two}\footnote{The capital /C/ stands for a plosive. As the numeral does not occur independently, and as it always assimilates to the following consonant, its place of articulation could not be determined.} and \emph{sum} \rede{three} are known. Another  numeral for \rede{one} is found, which is the \ili{Nepali} loan \emph{eko}. It already replaces the Yakkha numeral \emph{i} in several contexts. In counting, for instance,  \emph{eko} prevails in the majority of cases. Some fixed expressions, like \emph{i len} \rede{one day}, however, contain the Yakkha form.
\largerpage
 It is quite likely  that the numeral \emph{i} and the interrogative root \emph{i} share a common origin.

Unlike in some \ili{Newari} varieties,\footnote{For instance, in Dolakha \ili{Newari} \citep[220]{Genetti2007_Newari} and the \ili{Newari} spoken in Dulikhel (own observations).} numeral classification does not play a prominent role in Kiranti languages. Yakkha has one numeral classifier \emph{-paŋ} for human reference (cognate, e.g., with Belhare \emph{-baŋ}, Athpare \emph{-paŋ}, Camling \emph{-po}, \ili{Bantawa} \emph{-pok}, Hayu \emph{-pu}). It is  used only with the Yakkha \isi{numerals} \rede{two} and \rede{three}  (see \Next).  Nonsingular marking of the head noun is frequent, but optional (discussed in  \sectref{number}). For \isi{numerals} above \rede{three},  borrowed \ili{Nepali} \isi{numerals}, as well as the \ili{Nepali} classifiers \emph{jana} for humans and \emph{(w)oʈa} for things are used (see \NNext[a]). Some words for measuring units or currency may also function as classifiers (see \NNext[b]).  
...
Since there is no classifier for non-human reference in Yakkha, the nonsingular marker \emph{=ci} has undergone reanalysis in order to fill the  position of the classifier (see \Next). This is the only instance where nonsingular \emph{=ci} may occur inside a \isi{noun phrase}. 
...
Numeral expressions may also occur without a head noun; i.e., they can fill the structural position of a \isi{noun phrase} (see \Next).
...
Yakkha has a marker \emph{-ma} to individuate and count events, i.e., to express \rede{once}, \rede{twice}, \rede{three times}. It only occurs with the inherited (\isi{Tibeto-Burman}) Yakkha \isi{numerals}.
...
Yakkha interrogatives are based on the roots \emph{i} and \emph{heʔ}. \tabref{int-pron} provides an overview. While \emph{i} may also occur independently, with the meaning \rede{what} (referring to events, see \Next),  \emph{heʔ} always occurs with further morphological material. Some interrogatives are easily analyzable into a base plus \isi{case} marker, \isi{nominalizer} or \isi{clause linkage} marker, but others are not transparent. Interrogatives may also function as indefinite pronouns (see \sectref{sec-indef} above).
\largerpage 
...
The interrogatives \emph{ina/iya} and \emph{isa} may also head noun phrases (without modifiers), host nominal morphology and appear as predicates of interrogative copular clauses (see \Next). When a \isi{noun phrase} is headed by an interrogative, modifying material is not allowed, except for clauses in which the interrogatives have an indefinite interpretation (discussed above in \sectref{sec-indef}).  The quantifying/\isi{degree} interrogative \emph{ikhiŋ} (derived from the demonstrative base \emph{khiŋ} discussed in \sectref{sec-quant}) may also occur in noun-modifying position (see \NNext).
...
\rede{In which year was  your (my aunt's) marriage?} \source{06\_cvs\_01.031}
\bg.ikhiŋ   miʔwa hond-end-u-g=ha!\\
how\_much tear uncover{\sc -V2.insert-3.P[pst]-2.A=nmlz.nsg}\\
\rede{How many tears you have shed!}\footnote{The V2 \emph{-end} indicates transitive \isi{motion} downwards here.} \source{37\_nrr\_07.111}
...
The interrogative \emph{ikhiŋ} is furthermore often found in exclamations about size, amount or \isi{degree}, lacking the interrogative function (see \Next and \LLast[b]).
